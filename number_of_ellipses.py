...
# PMMA window 30..70 in X and Y (you can disable this if you want)
PMMA_X_MIN = 30.0
PMMA_X_MAX = 70.0
PMMA_Y_MIN = 30.0
PMMA_Y_MAX = 70.0
...
def ellipse_ok_in_world(world_ellipse) -> bool:
    ...
    You can return True here if you want NO prior.
    """
...
# =========================================================
# Per-image processing (ALL ellipses)
# =========================================================
def process_image(img_path, gt_ellipses):
    """
    Run detection on one image and return:
      - gt_count  = number of GT ellipses (primary + secondary)
      - det_count = number of detected ellipses
    """
    img = cv2.imread(img_path)
    if img is None:
        return len(gt_ellipses), 0
    ...
    gt_count = len(gt_ellipses)
    det_count = len(detected_ellipses)
    return gt_count, det_count


# =========================================================
# Main evaluation loop (count-based)
# =========================================================
def main():
    print(f"--- STARTING RHT (COUNT ALL ELLIPSES) ---")
    print(
    ...
    # group annotations by image id, KEEP ALL ellipses
    anns_by_id = {}
    for ann in data["annotations"]:
        anns_by_id[ann["image_id"]] = ann["ellipses"]
    ...
    for entry in data["images"]:
        ...
        gt_count, det_count = process_image(path, gt)
    ...
    mean_bias = (total_det - total_gt) / processed  # +ve = overcount, -ve = undercount
    exact_accuracy = exact_match_count / processed

    print("\n" + "=" * 30)
    print("FINAL COUNT-BASED RESULTS (ALL ELLIPSES)")
    print("=" * 30)
    print(f"Images:                     {processed}")
    print(f"Total GT ellipses:          {total_gt}")
    print(f"Total detected ellipses:    {total_det}")
    print("-" * 30)
    print(f"Exact Count Accuracy:       {exact_accuracy:.2%}  (images with correct #)")
